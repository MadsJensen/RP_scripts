...
# Save model
joblib.dump(gat, data_path + "decode_time_gen/%s_gat_ge.jl" % subject)
...
fig = gat.plot(
    ...
    subject)
fig.savefig(data_path + "decode_time_gen/%s_gat_matrix_ge.png" % subject)

fig = gat.plot_diagonal(
    chance=0.5,
    title="Temporal Gen (Classic vs planning): left to right")
fig.savefig(data_path + "decode_time_gen/%s_gat_diagonal_ge.png" % subject)
