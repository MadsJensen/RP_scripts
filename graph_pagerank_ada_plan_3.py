...
for subject in subjects:
    cls = np.load(source_folder + "graph_data/%s_classic_pow_plan.npy" %
                  subject).item()

    pln = np.load(source_folder + "graph_data/%s_plan_pow_plan.npy" %
                  subject).item()
...
for k, band in enumerate(bands.keys()):
    ...
    joblib.dump(ada_cv, source_folder +
                "graph_data/sk_models/pagerank_ada_plan_%s_3.plk" % band)

np.save(source_folder + "graph_data/pagerank_scores_all_plan_3.npy",
        scores_all)
