...
for condition in conditions:
    raw = Raw(mf_autobad_off_folder + "%s_%s_mc_tsss-raw.fif" % (subject,
                                                              condition),
              preload=True)
    ...
    raw.save(mf_autobad_off_folder + "%s_%s_filtered_mc_tsss-raw.fif" % (subject,
                                                               condition),
             overwrite=True)
    ...
    if eog_inds:   
        ...
        fig = ica.plot_components(eog_inds, title=title % ('eog', subject),
                                  colorbar=True)
        fig.savefig(ica_folder + "plots/%s_%s_eog_component_2.png" % (subject,
                                                                    condition))
        fig = ica.plot_overlay(eog_average, exclude=eog_inds, show=False)                                                                
        fig.savefig(ica_folder + "plots/%s_%s_eog_excluded_2.png" % (subject,
                                                                    condition))
    ...
    if ecg_inds:
        ...
        fig.savefig(ica_folder + "plots/%s_%s_ecg_component_2.png" % (subject,
                                                                    condition))
        fig = ica.plot_overlay(raw, exclude=ecg_inds, show=False)
        fig.savefig(ica_folder + "plots/%s_%s_ecg_excluded_2.png" % (subject,
                                                                   condition))
    ...
    raw_ica.save(ica_folder + "%s_%s_ica-raw.fif" % (
        subject, condition),
                 overwrite=True)
